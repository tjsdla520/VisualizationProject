...
CHART_NAME = 'ligue1TopFive'
#정보가 담긴 파일
filename = '../ligue1.csv'
################################################
data = pd.read_csv(filename, index_col='teamName')
TEAM = ['릴 OSC','파리 생제르맹 FC','AS 모나코 FC','올랭피크 리옹','올랭피크 드 마르세유']
...
stacked=False
yticks_interval = 5
data.plot(kind='bar', rot=10, title='ligue1 top 5 골득실차', legend=True, stacked=stacked)
plt.legend(loc='best')
if stacked == False:
