...
def getData():
    ...
    startYear = 2009
    endYear = 2021
    for page_idx in range(startYear, endYear):
        url = base_url
        url += '?category=epl'
        url += '&year=%s&tab=team' % str(page_idx)
        print('%s번째 페이지가 호출됨'% str(page_idx))

        # Soccerutil.py의 클래스 호출
        eplleague = leagueinfo(leagueName, url)
        # 셀리니움으로 웹페이지 크롤링
        soup = eplleague.getWebDriver()
        ...
        for mytd in mytrlists:
            ...
            if (len(mytrlists)) > 1:
                ...
                print(goalGap)
                saveData.append([page_idx, rank, teamName, gameCount, gainPoint, won, drawn, lost, gainGoal, loseGoal, goalGap])

    # epl리그 12년치 크롤링 결과물 저장
    eplleague.saveCsv(saveData)
